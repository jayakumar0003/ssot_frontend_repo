...
const TablePage = () => {
  ...
  // Get selected advertiser from navigation state
  const initialAdvertiser = location.state?.selectedAdvertiser || "";
  
  const [activeTab, setActiveTab] = useState("radia-plan");
  const [selectedAdvertiser, setSelectedAdvertiser] = useState(initialAdvertiser); // ADD THIS STATE
  ...
  // Method to load Radia Plan data
  const loadRadiaPlanData = async () => {
    try {
      ...
      setRadiaPlanError(
        "Failed to load Radia Plan data. Please try again later."
      );
  ...
  // Method to load Media Plan data
  const loadMediaPlanData = async () => {
    try {
      ...
      setMediaPlanError(
        "Failed to load Media Plan data. Please try again later."
      );
  ...
  // Method to load Campaign Overview data
  const loadCampaignOverviewData = async () => {
    try {
      ...
      setCampaignOverviewError(
        "Failed to load Campaign Overview data. Please try again later."
      );
  ...
  // Method to load Targeting & Analytics data
  const loadTargetingAnalyticsData = async () => {
    try {
      ...
      setTargetingAnalyticsError(
        "Failed to load Targeting & Analytics data. Please try again later."
      );
  ...
  // Load Radia Plan data on component mount
  useEffect(() => {
    loadRadiaPlanData();
  }, []);
        ...
        return (
          <div className="bg-white rounded-xl shadow-lg p-4">
            {radiaPlanLoading ? (
              ...
              <RadiaplanTable 
                data={radiaPlanData} 
                selectedAdvertiser={selectedAdvertiser} // PASS IT HERE
              />
            )}
          </div>
        );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-500/60 to-pink-500/50">
